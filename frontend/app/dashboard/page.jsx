...
function CircleStat({ label, value, subtitle, icon: Icon, ring }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-2 md:p-5 flex items-center gap-2 md:gap-4">
      <div
        className={`md:w-14 md:h-14 w-10 h-10 rounded-full border-2 flex items-center justify-center shrink-0 ${ringClasses[ring]}`}
      >
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                {stats.patientsPerDoctor.length === 0 ? (
                  <p className="text-sm text-slate-400 py-8 text-center">
                    No doctors ranked yet.
                  </p>
                ) : (
                  <div className="space-y-1">
                    {stats.patientsPerDoctor
                      .slice(0, 5)
                      .map((d, i) => (
                        <div
                          key={d.doctorId}
                  ...
                  <div className="space-y-3">
                    {stats.conditionBreakdown.map((c, i) => {
                      const pct = totalConditionCount
                        ? Math.round(
                            (c.count / totalConditionCount) * 100
                          )
                        : 0;
